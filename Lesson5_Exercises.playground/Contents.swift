...
func lastCharacterOf(word word: String)-> Character {
    return word[word.endIndex.predecessor()]
}


//:Test out your discovery below by returning the last letter of the String, "bologna".
var word = "bologna"
lastCharacterOf(word: word)
...
func combineLastCharacters( srcArray : [String])-> String{
    var output = ""
    for item in nonsenseArray {
        output.append(lastCharacterOf(word: item))
    }
    return output
}
