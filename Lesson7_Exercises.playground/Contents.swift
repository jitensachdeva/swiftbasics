...
enum Hand: Int{
    case thumb = 1, indexFinger, middleFinger, ringFinger, smallFinger
}

Hand.indexFinger.rawValue
...
struct Cookie{
    ...
    var delicious:Bool {
        get{
            if minutesSinceRemovalFromOven > 5{
                return false
            }else {
                return true
            }
...
var biscuit = Cookie(flavor: "Yummy", minutesSinceRemovalFromOven:6)
biscuit.delicious
biscuit.tempt()
...
class BnBListing {
    ...
    init(category: Category, available:Bool, rentPerDay:Double){
    ...
    func book() {
        available = !available
        print("Avalibility - \(available)")
    }
}

var room = BnBListing(category:Category.hotel, available:false, rentPerDay:12.0)
room.available
room.book()
room.available
//: __5b.__
